...
const BackDropsMovies = () => {
  const { details } = useSelector((state) => state.detailsMov);
  const { backDropsMovieslang, backDropsMoviesNolang, loding, erorr } =
    useSelector((state) => state.allImage);
  const navegate = useNavigate();
  const { movieid } = useParams();
  const dispatch = useDispatch();
  console.log(backDropsMoviesNolang);
  useEffect(() => {
        ...
        {!loding ? (
          <div className=" me-5">
            ...
            >
              ...
              <Tab
                eventKey="profile"
                title={`ENGLISH (${backDropsMovieslang.length})`}
              >
                <div className="col-12 d-flex justify-content-center flex-wrap gap-4 bg-dark">
                  {backDropsMovieslang &&
                    backDropsMovieslang.map((nolang, index) => (
                      <img
                        src={`https://image.tmdb.org/t/p/w600_and_h900_bestv2/${
                          nolang && nolang.file_path
                        }`}
...
export default BackDropsMovies;
